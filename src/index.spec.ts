...
describe("traverseObject", () => {
  ...
  it("clones by default", () => {
    ...
    let returnedObj = traverseObject(originalObj, (key, prop) => {
      prop.value = 3;
    });
  ...
  it("will recurse if desired", () => {
    let originalObj = { one: { value: 1 }, two: { value: 2 } };
    let returnedObj = traverseObject(
      originalObj,
      (key, prop) => [key + "2", prop],
      true
    );
...
describe("nestedPropertyDetails", () => {
  ...
  it("marks a missing property as false", () => {
    let obj = { level1: { level2: "I am here" } };
    expect(nestedPropertyDetails(obj, "level1.level2.level3").exists).toEqual(
      false
    );
  });
  it("provides the existing path", () => {
    let obj = { level1: { level2: "I am here" } };
    expect(
      nestedPropertyDetails(obj, "level1.level2.level3.level4").existingPath
  ...
  it("can traverse circular objects", () => {
    ...
    expect(
      nestedPropertyDetails(obj, "level1.original.level1.original")
        .finalValidProperty
    ).toEqual(obj);
  });
});
...
describe("nestedPropertyTest", () => {
  ...
  it("returns false if a prop does not exist at some level", () => {
    ...
    expect(nestedPropertyTest(obj, "level1.level2.level3", () => true)).toEqual(
      false
    );
  });
  it("returns true for a successfull callback on an existing prop", () => {
    ...
    expect(
      nestedPropertyTest(obj, "level1.level2", (val) => val === "I am here")
    ).toEqual(true);
  });
  it("returns false for an unsuccessful callback on an existing prop", () => {
    ...
    expect(
      nestedPropertyTest(obj, "level1.level2", (val) => val === "I am not here")
    ).toEqual(false);
  });
});
...
describe("nestedPropertyOrDefault", () => {
  ...
  describe("when the property exists", () => {
    it("returns the property", () => {
      ...
      expect(nestedPropertyOrDefault(nested, "level1.level2")).toEqual(
        "I am here"
      );
    });
  });
  describe("when the property is absent", () => {
    it("returns null", () => {
      const nested = { level1: { level2: "I am here" } };
      expect(nestedPropertyOrDefault(nested, "level1.level2.level3")).toEqual(
        null
      );
    });
    it("returns the default supplied", () => {
      const nested = { level1: { level2: "I am here" } };
      expect(
        nestedPropertyOrDefault(nested, "level1.level2.level3", "default value")
